...
- Add to `PROFILE_DEFAULTS` as `autoCollectReward = 1` (1–4 = index)
...
## Shift-Click Invert Option

- Currently: auto-accept is on by default, hold Shift to bypass (manual gossip)
- **New option:** invert the behavior — click Astalor normally to open gossip manually, Shift+click to trigger auto-hunt
- Useful for players who want auto-accept as a "sometimes" shortcut rather than the default
- Setting: `shiftInvert = false` in `PROFILE_DEFAULTS`
- Logic in `PreyMate_Accept.lua`: `if profile.shiftInvert then autoAccept = IsShiftKeyDown() else autoAccept = not IsShiftKeyDown() end`
- Update the shift-click hint text in the options panel to reflect current mode
