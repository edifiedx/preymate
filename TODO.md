...
Open questions are marked **[?]**. Items are loosely ordered by priority.
...
- Need to discover the correct API call. Candidates to probe in-game:
  ```lua
  /run AdventureJournal_OpenToEntry(?)
  /run ToggleEncounterJournal()
  /run EventRegistry:TriggerEvent("OPEN_SEASONAL_CONTENT_PANEL") -- may not exist
  /run C_SeasonInfo.GetCurrentSeasonID()  -- check if Prey has a season record
  ```
...
- One-time popup on login when new settings are added
- Track via `CONFIG_VERSION` integer in `PreyMate.lua` + `PreyMateDB.lastSeenConfigVersion`
- Fire after `PLAYER_ENTERING_WORLD`; "Later" re-shows next login until they open settings
...
- **Voidlight Marl balance in tooltip** — currency ID unknown; add once confirmed in-game

---

## Confirmed APIs & Findings

- `C_QuestLog.GetActivePreyQuest()` → returns active hunt quest ID ✅
- `C_CurrencyInfo.GetCurrencyInfo(3392)` → Remnants of Anguish (confirmed ID, no cap) ✅
- `ShowQuestComplete(questID)` + `C_Timer.After(delay)` + `GetQuestReward(index)` → auto-complete flow ✅
- `GetNumQuestChoices()` → 0 for table quests, 4 for random hunts ✅
- Reward grid (2×2, row-major): Gold=1, Marl=2, Dawncrest=3, Anguish=4 ✅
- Quest line ID `5945` → identifies Prey hunt quests via `C_QuestLine.GetQuestLineQuests()` ✅
- Trap item ID `255825`, max stack 5 ✅
- `QUEST_LOG_UPDATE` O(1) early-exit optimization — implemented ✅
- Gossip option ID `134357` → Prey hunt option on Astalor ✅
